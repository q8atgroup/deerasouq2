...
if (!function_exists('update_version')):
    ...
    {
        if (settingHelper('current_version') == '1.0.0' || settingHelper('current_version') < 154):

            if (is_dir(base_path('public/frontend/js/chunks-153')))
            {
                \Illuminate\Support\Facades\File::deleteDirectory(base_path('public/frontend/js/chunks-153'));
            }
    ...
    function validate_purchase($code, $data)
    {
        ...
        if ($curl_info["http_code"] == "200"):
            ...
            if ($curl_response->status):
                ...
                if ($curl_response->sql_data == ''):
                    return 'Please check you purchase code and try again, or contact with our support.';
                endif;
                ...
                if (is_dir(base_path('public/sql')) == false):
                    try {
...
if (!function_exists('priceFormatUpdate')):
    function priceFormatUpdate($price,$curr,$type=null)
    {
        ...
        if ($type == '*') {
            $amount = round($price*$rate,settingHelper('no_of_decimals'));
        } else {
            $amount = $price/$rate;
        }
        return $amount;
...
if (! function_exists('mixUrl')) {
    ...
    if (!function_exists('default_country')):
        ...
        {
            $country_code = Country::where('id',$id)->first();
            return $country_code->iso2;
        }
    endif;
}
